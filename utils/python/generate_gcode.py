...
def load_config_from_json(config_file="/usr/local/src/elevation-simple/config/dmapConfig.json"):
    """
...
def create_default_config():
    ...
    if json_config:
        ...
        gpre_commands = advanced.get('gpre', ['G17 G90', 'M3 S24000', 'G4 P5'])
        if isinstance(gpre_commands, list):
            config.gpre.set('|'.join(gpre_commands))
        else:
        ...
        # G-code prefix and suffix
        config.gpre.set("G17 G90|M3 S24000|G4 P5")
...
def main():
    ...
    default_h3index = "default" # Default H3 index
    default_pass_type = "rough" # Default pass type
    ...
    if len(sys.argv) >= 4:
        ...
        h3index= sys.argv[3]
        pass_type = sys.argv[4]
    elif len(sys.argv) == 3:
        image_path = sys.argv[1]
        output_path = sys.argv[2]
        h3index = default_h3index
        pass_type = default_pass_type
    elif len(sys.argv) == 2:
        image_path = sys.argv[1]
    ...
    if pass_type == "rough" or pass_type == "both":
        ...
        try:
            ...
            ouput_file = output_path + '/' + h3index + '-rough.gcode'

            save_gcode(rough_gcode, ouput_file)
        except Exception as e:
            import traceback
    ...
    if pass_type == "finish" or pass_type == "both":
        ...
        try:
            ...
            if finish_gcode:
                ...
                ouput_file = output_path + '/' + h3index + '-finish.gcode'

                save_gcode(finish_gcode, ouput_file)
            else:
